...
export function HeroSection() {
  ...
  return (
    <section className="relative px-6 pt-16 pb-6 md:pt-24 md:pb-8 max-w-6xl mx-auto">
      ...
      <p className="font-serif-body text-cream-muted text-lg md:text-xl mt-4 mb-7 md:mb-8">
        {TRIP_DATA.dates}
      </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {quickStats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-xl border border-forest-green/30 bg-dark-card/80 px-4 py-3"
          >
            <p className="text-[10px] uppercase tracking-[0.2em] text-cream-muted/70">
              {stat.label}
            </p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <a
          ...
          className="rounded-full border border-forest-green/35 bg-dark-card px-3 py-1.5 text-xs font-medium uppercase tracking-[0.15em] text-cream-muted hover:text-cream hover:border-amber/40 transition-colors"
        >
          Booking Queue
        </a>
        <a
